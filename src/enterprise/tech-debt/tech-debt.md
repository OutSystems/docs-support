...
*   **Poor service abstraction**: Services for core business concepts are not correctly isolated and abstracted. Business rules are spread over different systems, making them hard to reuse, and code reuse has very little structure.
*   **Unmanageable dependencies**: Components of the system are not correctly isolated from each other, so updating or replacing a system has a snowball effect on other systems.
