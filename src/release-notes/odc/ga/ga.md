...
<ul>
<li>Traces - Fixed an issue that caused the Span's logs to disappear when the option "Show all" was clicked on the Trace detail screen. (RDLOT-1644)</li>
<li>Traces - Fixed an issue that led the Trace detail page to not load the information when the page was opened in a new tab. (RDLOT-1649)</li>
</ul>

<div class="hidden" id="outsystems-developer-cloud-2023-03-09_end"></div><div class="hidden" id="outsystems-developer-cloud-2023-03-02_start"></div>
...
<ul>
<li>When reporting an issue, you now have the option to select the default timezone for your organization for follow up from Support (RDCCP-337)</li>
<li>It is now possible to manage the contacts that should be included by email of an existing support case. (RDCCP-373)</li>
<li>When reporting an issue now it’s possible to specify the steps to replicate, separated from the issue description. (RDCCP-376)</li>
<li>Apps - Now it’s possible to delete apps deployed in non-production stages. Deleting the app is still not possible if the app is deployed in production.  (RDLOT-1657)</li>
</ul>
<h3 id="bug_fixing_outsystems_developer_cloud_2023-03-02" >Bug Fixing</h3>
<ul>
<li>Apps - Fixed an issue that wouldn’t redirect the user to the login page when accessing the Mobile distribution screen without an active login session. (RDLOT-1640)</li>
</ul>

<div class="hidden" id="outsystems-developer-cloud-2023-03-02_end"></div><div class="hidden" id="outsystems-developer-cloud-2023-02-17_start"></div>
...
<ul>
<li>Monitoring - Added icons on the elements filter on the Traces page. (RDLOT-1503)</li>
<li>Monitoring - The trace detail page now supports displaying traces with a higher amount of spans. (RDLOT-1610)</li>
<li>From now on, when managing Secure Gateways in the ODC Portal, we will only provide you with the authentication token when it is generated. This happens when you activate a Secure Gateway for the first time ever, or when you renew the token. When the token is generated, you will be informed to copy the token and store it somewhere secure, so you don’t lose it. (RPOR-13372)</li>
</ul>
<h3 id="bug_fixing_outsystems_developer_cloud_2023-02-17" >Bug Fixing</h3>
<ul>
<li>Monitoring - The time in the Related logs table is now presented in the client timezone, both in the Log and Trace detail pages. (RDLOT-1616)</li>
<li>ODC Portal Organization roles enabled pagination on the list page. Now, it contains 20 organization roles per page and will help to view the information better. (RDPP-1078)</li>
</ul>

<div class="hidden" id="outsystems-developer-cloud-2023-02-17_end"></div><div class="hidden" id="outsystems-developer-cloud-2023-02-16_start"></div>
...
<p>The ODC User with Manage organization roles can change the mapping from picture to another alternative mapping inside the External IdP configuration screen.</p>
<p>The picture is now available in the ODC profile and user list, and will be soon available in the Apps as well. (RDPP-818)</p></li>
</ul>
