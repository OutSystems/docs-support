...
Information about active Public EAPs will be shared [on the EAP page](https://www.outsystems.com/eap/).
...
Beta releases include non-final versions of a product capability. Unlike EAP or LA programs, they are made globally available to all eligible customers to gather broad feedback and improve product-market fit, so features may change significantly or be discontinued. Beta releases can range from individual capabilities to an entire product version, such as MABS.
...
* **Platform Server**: The core server part of OutSystems product. A new version of Platform Server is released twice a quarter and you can check the [latest Platform Server releases](https://www.outsystems.com/downloads/search/Platform-Server/11).

* **LifeTime management console**: The web console for DevOps processes. LifeTime is released monthly and you can check the [latest LifeTime releases](https://www.outsystems.com/downloads/search/LifeTime/11).

* **Service Studio**: The visual IDE, Service Studio has weekly releases. Check the [latest Service Studio releases](https://www.outsystems.com/downloads/search/Service-Studio/11).

* **Supported Forge components**: Components that increase the existing built-in capabilities and can be reused across applications. Each component has its own release cycle. OutSystems components can be found [on the Forge](https://www.outsystems.com/forge/list?q=&t=&o=most-popular&tr=False&oss=True&c=%20&a=&v=&hd=False&tn=&scat=forge).

The following products are delivered as Software as a Service over the web and continuously updated and improved:

* **Mobile Apps Build Service**: A cloud service developed by OutSystems that allows the generation of mobile app packages. Check out the [MABS release history](mabs/mabs-versions.md).

* **AI Mentor Studio**: The OutSystems technical debt monitoring tool, delivered as a cloud service. AI Mentor Studio release history can be found [on the AI Mentor Studio release notes](https://success.outsystems.com/support/release_notes/ai_mentor_studio/).
